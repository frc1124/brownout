...
public class PidDrive extends PIDSubsystem {
  ...
  public PidDrive(MotorControllerGroup motors, Encoder encoder, AHRS navx, PIDController controller, boolean isLeft) {
    ...
    this.isLeft = isLeft;
    this.encoder.setDistancePerPulse(10); // Change (inches per pulse)
  ...
  public void useOutput(double output, double setpoint) {
    ...
    double angleError = navx.getRate();
    
    if (isLeft) {
      out += angleError * Constants.LEFT_ANGLE_P;
    } else {
      out -= angleError * Constants.RIGHT_ANGLE_P;
    }    
